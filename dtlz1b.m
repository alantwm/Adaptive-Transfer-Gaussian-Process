...
b1=b(1);b2=b(2);
M=2;
x=x'; % modified to be compatable with TrParEGo

% k = 7; %as suggested by Deb
k=size(x,1)-(M-1);
...
g = (100+b1)*(k+b2 + sum((xm - 0.5).^2 - cos(2*pi*(xm - 0.5)),1));
...
f(M,:) = 1/2*(1 - x(1,:)).*(1 + g);
f=f'; % modified to be compatable with TrParEGo